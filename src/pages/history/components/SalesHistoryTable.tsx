...
export const SalesHistoryTable: React.FC<SalesHistoryTableProps> = React.memo(({
  ...
  const getPaymentStatus = (sale: any) => {
    if (sale.isPaidInCash) {
      return { label: t("history.paidInCash", "Paid in Cash"), variant: "default" as const };
    }
    if (sale.remainingAmount > 0) {
      return { label: t("history.partiallyPaid", "Partially Paid"), variant: "secondary" as const };
    }
    return { label: t("history.fullyPaid", "Fully Paid"), variant: "default" as const };
  };
      ...
      <div className="rounded-lg border bg-card p-8 text-center">
        ...
        <p className="text-muted-foreground">
          {dateRange.startDate || dateRange.endDate
            ? t("history.noSalesInPeriod", "No sales found in the selected period")
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="relative w-80">
          ...
          <Input
            placeholder={t("history.searchSales", "Search sales by client or ID...")}
            value={searchTerm}
              ...
              {currentData.map((sale) => {
                ...
                return (
                  <tr key={sale.id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="p-4">
                      <div className="text-sm font-medium">
                        {sale.client?.name || t("history.noClient", "No Client")}
                      </div>
                      {sale.client?.phone && (
                        <div className="text-xs text-muted-foreground">
                          {sale.client.phone}
