...
export default function History() {
  ...
  useEffect(() => {
    ...
    if (paymentsLoaded && rawPaymentsData.length > 0) {
      const filteredPayments = filterDataByDateRange(
        rawPaymentsData,
        dateRange,
      );
      ...
      const totalPaymentAmount = filteredPayments.reduce(
        (sum, payment) => sum + payment.givenAmount,
        0,
      );
      const totalPaidAmount = filteredPayments.reduce((sum, payment) => {
        // Only count payments that have been actually paid (have a paidDate)
        return payment.paidDate ? sum + payment.givenAmount : sum;
      }, 0);
      setStats((prev) => ({
        ...prev,
  ...
  const loadPaymentsData = async () => {
    ...
    try {
      const payments =
        await window.api.database.payments.getAllWithClientInfo();
