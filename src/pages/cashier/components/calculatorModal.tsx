...
const opButton =
  "border border-border bg-card text-foreground font-bold";
const opButtonActive =
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center transition-all duration-300">
      ...
      <div className="relative z-10 bg-card rounded-2xl shadow-2xl border border-border p-6 min-w-[340px] max-w-[95vw] flex flex-col items-center animate-fadeIn">
        ...
        <div className="flex w-full items-center justify-between mb-2">
          <div className="flex-1 text-center text-lg font-bold text-foreground select-none">Calculator</div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-row w-full justify-between mb-2">
          <button className={memButton} onClick={() => handleMemory("M+")} aria-label="Memory add">M+</button>
          <button className={memButton} onClick={() => handleMemory("M-")} aria-label="Memory subtract">M-</button>
          <button className={memButton} onClick={() => handleMemory("MR")} aria-label="Memory recall">MR</button>
          <button className={memButton} onClick={() => handleMemory("MC")} aria-label="Memory clear">MC</button>
        </div>
        {/* Expression display */}
        ...
        <div className="grid grid-cols-4 gap-2 mt-1">
          ...
          {/* Second row: 7, 8, 9, × */}
          <button className={buttonBase} onClick={() => handleInput("7")} aria-label="7">7</button>
          <button className={buttonBase} onClick={() => handleInput("8")} aria-label="8">8</button>
          <button className={buttonBase} onClick={() => handleInput("9")} aria-label="9">9</button>
          <button className={`${buttonBase} ${opButton}${lastOp === "*" ? ` ${opButtonActive}` : ""}`} onClick={() => handleInput("*")} aria-label="Multiply">×</button>

          {/* Third row: 4, 5, 6, - */}
          <button className={buttonBase} onClick={() => handleInput("4")} aria-label="4">4</button>
          <button className={buttonBase} onClick={() => handleInput("5")} aria-label="5">5</button>
          <button className={buttonBase} onClick={() => handleInput("6")} aria-label="6">6</button>
          <button className={`${buttonBase} ${opButton}${lastOp === "-" ? ` ${opButtonActive}` : ""}`} onClick={() => handleInput("-")} aria-label="Subtract">-</button>

          {/* Fourth row: 1, 2, 3, + */}
          <button className={buttonBase} onClick={() => handleInput("1")} aria-label="1">1</button>
          <button className={buttonBase} onClick={() => handleInput("2")} aria-label="2">2</button>
          <button className={buttonBase} onClick={() => handleInput("3")} aria-label="3">3</button>
          <button className={`${buttonBase} ${opButton}${lastOp === "+" ? ` ${opButtonActive}` : ""}`} onClick={() => handleInput("+")} aria-label="Add">+</button>

          {/* Fifth row: (, 0, ., = */}
          <button className={`${buttonBase} ${funcButton}`} onClick={() => handleInput("(")} aria-label="Left parenthesis">(</button>
          <button className={buttonBase} onClick={() => handleInput("0")} aria-label="0">0</button>
          <button className={buttonBase} onClick={() => handleInput(")")} aria-label="Right parenthesis">)</button>
          <button className={`${buttonBase} bg-primary text-secondary font-bold hover:bg-primary/90`} onClick={handleEquals} aria-label="Equals">=</button>
        </div>
        <div className="mt-4 text-xs text-muted-foreground w-full text-right tracking-wide">Memory: {memory}</div>
      </div>
      <style>{`
