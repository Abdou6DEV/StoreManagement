...
export const BadgeNotification: React.FC<BadgeNotificationProps> = ({
  ...
  const isCompact = displayCount === `${maxCount}+`;
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "absolute top-0 right-0.5 rtl:right-auto rtl:left-0.5 h-[18px] flex items-center justify-center text-xs font-bold text-white transform translate-x-1/2 -translate-y-1/2 rtl:translate-x-[-50%] min-w-[16px]",
        isCompact ? "px-1 min-w-0 text-[11px]" : "px-1.5",
