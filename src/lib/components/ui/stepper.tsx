  ...
  const store = React.useMemo<Store>(() => {
    return {
      ...
      addStep: (value, completed, disabled) => {
        const existing = stateRef.current.steps.get(value);
        if (existing) {
          stateRef.current.steps.set(value, { ...existing, completed, disabled });
        } else {
          stateRef.current.steps.set(value, { value, completed, disabled });
...
function StepperTrigger(props: ButtonProps) {
  ...
  }, [focusContext.onFocusableItemAdd, focusContext.onFocusableItemRemove, isDisabled]);
