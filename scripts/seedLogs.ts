...
class StandaloneLogger {
  ...
  log(level: LogLevel, message: string, context?: string, data?: any, userId?: string, process: 'main' | 'renderer' = 'main'): void {
  ...
  error(message: string, context?: string, data?: any, userId?: string, process?: 'main' | 'renderer'): void {
    this.log(LogLevel.ERROR, message, context, data, userId, process);
  }

  warn(message: string, context?: string, data?: any, userId?: string, process?: 'main' | 'renderer'): void {
    this.log(LogLevel.WARN, message, context, data, userId, process);
  }

  info(message: string, context?: string, data?: any, userId?: string, process?: 'main' | 'renderer'): void {
    this.log(LogLevel.INFO, message, context, data, userId, process);
  }

  debug(message: string, context?: string, data?: any, userId?: string, process?: 'main' | 'renderer'): void {
    this.log(LogLevel.DEBUG, message, context, data, userId, process);
  }

  trace(message: string, context?: string, data?: any, userId?: string, process?: 'main' | 'renderer'): void {
...
class LogMessageGenerator {
  private static contexts = [
    'auth', 'database', 'api', 'ui', 'payment', 'inventory', 'sales', 'client', 'product', 'session'
  ];
    ...
    switch (level) {
      ...
      case LogLevel.INFO:
        ...
        data = {
          ...
          resource: faker.helpers.arrayElement(['product', 'client', 'sale', 'payment'])
        };
        break;
      case LogLevel.DEBUG:
      ...
      case LogLevel.TRACE:
        ...
        data = {
          functionName: faker.helpers.arrayElement(['processPayment', 'validateInput', 'updateInventory', 'generateReport']),
                   lineNumber: faker.number.int({ min: 1, max: 100 }),
         executionTime: faker.number.float({ min: 0.1, max: 10.0, fractionDigits: 1 })
        };
        break;
    }
...
async function seedLogs(options: {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (timeRange) {
      logEntry.timestamp = faker.date.between({
